...
def spieltag(tag):
  ...
  spielergen1 = coltoArr5("Trainer1Gen", "Trainername",spielergen1)  #liste aller Trainer aus Gen1
  ...
  if spieltage2[0] == 1:
    ...
    result11 = generate_round_robin(spielergen1[:-20], 1,tag3,allespieler)  # Alle Win lose Results
    result12 = generate_round_robin(spielergen1[10:20], 1,tag3,allespieler)  # Alle Win lose Results
    result13 = generate_round_robin(spielergen1[20:], 1, tag3,allespieler)
    ...
    for i in range(5):
      wintodb("SpieltagGen11", "Field" + str(i + 1), result11[i], "ROWID",tag-tag3)
      wintodb("SpieltagGen12", "Field" + str(i + 1), result12[i], "ROWID",tag-tag3)

      wintodb("SpieltagGen13", "Field" + str(i + 1), result13[i], "ROWID",tag-tag3)
        ...
        if table1[i][0] == spieler[e][3]:
          if spieler[e][4] == 0:  #hat er schon Tranfer gemacht?
            if spieler[e][5] != None:  #Will er überhaupt nen Transfer machen?
              #print(spieler[e][1])  #def pokeaccess1(trainername, gen, pokemon1, pokemon2):
  ...
  for i in range(
    ...
    if gen == i + 1:
      ...
      if 1 / 3 * len(mtable[i]) < index <= 2 / 3 * len(mtable[i]):  # Liga2
        print("Liga2")
        temptable = mtable[i][int(1 / 3 * len(mtable[i])):int(2 / 3 *len(mtable[i]))]
